...
class Magician(Character):
	...
	@mp.setter
	def mp(self, val: int):
		self.mp = utils.clamp(val, 0, self.max_mp)
	...
	def spell(self, spell):
		if spell is not None and spell.min_level > self.level:
			raise ValueError()
	...
	def _compute_magical_damage(self, other):
		return Character.compute_damage_output(self.level + self.magic_attack, self.spell.power, 1, 1, 1 / 16, (0.85, 1.0))
